...
{
    // 1. Validate dữ liệu (Thêm validate cho phone luôn cho an toàn)
    $request->validate([
        'name'     => 'required',
        'email'    => 'required|email|unique:users',
        'phone'    => 'nullable|max:20', // Thêm dòng này: Cho phép để trống, tối đa 20 ký tự
    ...
    try {
        ...
        return redirect()->route('user.index')
               ->with('status_success', 'Tạo mới người dùng thành công!');
    } catch (\Exception $e) {
        // dd($e->getMessage());
        Log::error($e);
        return redirect()->route('user.index')
               ->with('status_error', 'Xảy ra lỗi khi thêm người dùng!');
    }
}
