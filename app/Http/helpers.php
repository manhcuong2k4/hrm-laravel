...
if(! function_exists('getTotalOfNumberStaff')){
    function getTotalOfNumberStaff($status){
        ...
        if($status == -1){
            $total = App\Models\NhanSu::all()->count();
        }elseif($status == 0){
            $total = App\Models\NhanSu::where('trang_thai', 0)->get()->count();
        }elseif($status == 1){
            $total = App\Models\NhanSu::where('trang_thai', 1)->get()->count();
        }
...
if(! function_exists('getTenPhongBanById')){
    function getTenPhongBanById($phongban_id){
        if($phongban_id > 0){
            $tenphongban = App\Models\PhongBan::findOrFail($phongban_id)->ten;
        }else{
            $tenphongban = '';
        }
        return $tenphongban;
